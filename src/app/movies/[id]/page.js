        ...
        <Link href={`/movies/${movie.id}`}>
            ...
            >
                <div className="relative">
                    {movie.poster_path ? (
                        ...
                        <div className="aspect-[2/3] bg-gradient-to-br from-base-300 to-base-200 flex items-center justify-center">
                            <FiImage className="text-3xl text-base-content/50" />
                        </div>
                    )}
                ...
                <div className="flex flex-col flex-grow p-3 sm:p-4">
                    <p className="text-sm font-medium transition-colors duration-300 line-clamp-1 group-hover:text-primary sm:text-base">
                        {movie.title}
                    </p>
...
const MovieDetailsPage = () => {
    ...
    const movieEmbedUrl = `https://cinesrc.st/embed/movie/${id}`;

    const generateMockTorrentLinks = (movieTitle, year) => {
        // Generate mock torrent data for demo purposes
        const baseHash = movieTitle.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
        const hashSuffix = year.toString();

        return [
            {
                quality: "1080p",
                size: "2.1 GB",
                hash: `${baseHash}1080p${hashSuffix}`.padEnd(40, '0'),
                seeds: Math.floor(Math.random() * 500) + 100,
                peers: Math.floor(Math.random() * 50) + 10
            },
            {
                quality: "720p",
                size: "1.4 GB",
                hash: `${baseHash}720p${hashSuffix}`.padEnd(40, '0'),
                seeds: Math.floor(Math.random() * 300) + 50,
                peers: Math.floor(Math.random() * 30) + 5
            },
            {
                quality: "480p",
                size: "800 MB",
                hash: `${baseHash}480p${hashSuffix}`.padEnd(40, '0'),
                seeds: Math.floor(Math.random() * 200) + 25,
                peers: Math.floor(Math.random() * 20) + 3
            }
        ];
    };
    ...
    useEffect(() => {
        const fetchMovieDetails = async () => {
            try {
                ...
                // Generate mock torrent links
                if (movieData.title && movieData.release_date) {
                    const year = new Date(movieData.release_date).getFullYear();
                    const torrents = generateMockTorrentLinks(movieData.title, year);
            ...
            <div className="container px-4 py-8 mx-auto">
                {isLoading ? (
                    <div className="space-y-8">
                        ...
                        <div className="relative overflow-hidden shadow-2xl rounded-2xl">
                            <div className="relative h-[50vh] xs:h-[55vh] sm:h-[60vh] md:h-[70vh] w-full overflow-hidden rounded-xl bg-base-300 animate-pulse">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 z-20 p-4 sm:p-6 md:p-12">
                                    <div className="flex flex-col items-start gap-4 md:flex-row md:items-end sm:gap-6 md:gap-8">
                                        {/* Poster skeleton - hidden on mobile */}
                                        <div className="hidden shadow-xl md:block w-52 h-80 rounded-xl bg-base-200 animate-pulse"></div>

                                        {/* Movie info skeleton */}
                                        <div className="flex-1 space-y-4 sm:space-y-6">
                                            {/* Title skeleton */}
                                            <div className="w-3/4 rounded-lg h-7 sm:h-8 md:h-12 bg-base-200 animate-pulse"></div>

                                            {/* Metadata skeleton */}
                                            <div className="flex flex-wrap gap-2 sm:gap-3">
                                                <div className="w-20 h-6 rounded-full sm:w-24 bg-white/10 animate-pulse"></div>
                                                <div className="w-16 h-6 rounded-full sm:w-20 bg-white/10 animate-pulse"></div>
                                                <div className="w-24 h-6 rounded-full sm:w-32 bg-white/10 animate-pulse"></div>
                                            </div>

                                            {/* Genres skeleton */}
                                            <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                                {[...Array(3)].map((_, i) => (
                                                    <div key={i} className="w-16 h-6 rounded-full sm:h-7 sm:w-24 bg-primary/30 animate-pulse"></div>
                                                ))}
                                            </div>

                                            {/* Overview skeleton */}
                                            <div className="space-y-1.5 sm:space-y-2">
                                                <div className="w-full h-3 rounded sm:h-4 bg-white/10 animate-pulse"></div>
                                                <div className="hidden w-5/6 h-3 rounded sm:h-4 bg-white/10 animate-pulse sm:block"></div>
                                                <div className="hidden w-4/6 h-3 rounded sm:h-4 bg-white/10 animate-pulse md:block"></div>
                                            </div>

                                            {/* Buttons skeleton */}
                                            <div className="flex flex-wrap gap-2 sm:gap-4">
                                                <div className="w-12 h-12 rounded-full xs:h-10 xs:w-36 sm:h-12 sm:w-36 bg-primary/30 animate-pulse"></div>
                                                <div className="w-12 h-12 rounded-full xs:h-10 xs:w-48 sm:h-12 sm:w-48 bg-secondary/30 animate-pulse"></div>
                                                <div className="w-12 h-12 rounded-full xs:h-10 xs:w-36 sm:h-12 sm:w-36 bg-accent/30 animate-pulse"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 sm:gap-8">
                            ...
                            <div className="space-y-4 md:col-span-2 sm:space-y-6">
                                {/* Cast Section */}
                                <div className="p-4 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-xl sm:rounded-2xl sm:p-6 border-base-content/5">
                                    <div className="flex items-center justify-between mb-4 sm:mb-6">
                                        <div className="w-24 h-6 rounded sm:h-8 sm:w-32 bg-base-300 animate-pulse"></div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 sm:gap-4">
                                        {[...Array(8)].map((_, i) => (
                                            <div key={i} className="flex items-center gap-2 p-2 sm:gap-3">
                                                <div className="w-10 h-10 rounded-full sm:w-12 sm:h-12 bg-base-300 animate-pulse"></div>
                                                <div className="flex-1 space-y-1.5 sm:space-y-2">
                                                    <div className="w-20 h-3 rounded sm:h-4 sm:w-24 bg-base-300 animate-pulse"></div>
                                ...
                                <div className="p-4 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-xl sm:rounded-2xl sm:p-6 border-base-content/5">
                                    <div className="flex items-center justify-between mb-4 sm:mb-6">
                                        <div className="w-32 h-6 rounded sm:h-8 sm:w-48 bg-base-300 animate-pulse"></div>
                                    </div>
                            ...
                            <div className="space-y-4 sm:space-y-6">
                                <div className="p-4 space-y-3 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-xl sm:rounded-2xl sm:p-6 border-base-content/5 sm:space-y-4">
                                    {[...Array(5)].map((_, i) => (
                                        <div key={i} className="space-y-1.5 sm:space-y-2">
                                            <div className="w-20 h-3 rounded sm:h-4 sm:w-24 bg-base-300 animate-pulse"></div>
                                            <div className="w-32 h-4 rounded sm:h-6 sm:w-48 bg-base-300 animate-pulse"></div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="relative overflow-hidden shadow-2xl rounded-2xl">
                            {movie.backdrop_path ? (
                                <div className="relative h-[50vh] xs:h-[55vh] sm:h-[60vh] md:h-[70vh] w-full overflow-hidden rounded-xl">
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 z-20 p-4 sm:p-6 md:p-12">
                                        <div className="flex flex-col items-start gap-4 md:flex-row md:items-end sm:gap-6 md:gap-8">
                                            ...
                                            <div className="hidden overflow-hidden transition-transform duration-300 transform -translate-y-8 border-2 shadow-2xl md:block w-52 h-80 rounded-xl hover:scale-105 border-white/10 group">
                                                {movie.poster_path ? (
                                                    <img
                                            ...
                                            <div className="flex-1 text-white">
                                                <h1 className="pb-1 mb-2 text-2xl font-bold leading-normal tracking-tight text-transparent sm:text-3xl md:text-5xl sm:mb-3 bg-clip-text bg-gradient-to-r from-white to-white/80">
                                                    {movie.title}
                                                    {movie.release_date && (
                                                        <span className="ml-2 text-xl font-normal text-white/70 sm:text-2xl">
                                                ...
                                                <div className="flex flex-wrap items-center gap-2 mb-3 text-sm sm:gap-3 sm:mb-5 md:text-base">
                                                    {movie.release_date && (
                                                ...
                                                <p className="max-w-3xl mb-4 text-sm leading-relaxed text-white/90 sm:mb-7 line-clamp-3 md:line-clamp-none md:text-base">
                                                    {movie.overview}
                                                </p>
                                                ...
                                                <div className="flex flex-wrap gap-2 sm:gap-4">
                                                    ...
                                                    <button
                                                        ...
                                                        xs:w-auto w-12 h-12 xs:h-auto xs:px-4 xs:py-2.5 ${isInWatchlist
                                ...
                                <div className="p-4 border shadow-xl bg-gradient-to-br from-base-300 to-base-200 sm:p-8 md:p-10 rounded-2xl border-base-content/5">
                                    <div className="flex flex-col gap-8 md:flex-row md:gap-10">
                                        ...
                                        <div className="w-48 h-72 md:w-56 md:h-80 rounded-xl overflow-hidden shadow-xl bg-base-200 border border-base-content/10 group transition-transform duration-300 hover:scale-[1.02]">
                                            {movie.poster_path ? (
                                                ...
                                                <div className="flex items-center justify-center w-full h-full bg-gradient-to-br from-base-300 to-base-200">
                                                    <span className="font-medium text-base-content/50">No Poster Available</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex-1 space-y-5">
                                            <div>
                                                <h1 className="mb-2 text-3xl font-bold tracking-tight md:text-4xl lg:text-5xl">
                                                    {movie.title}
                                                    {movie.release_date && (
                                                        <span className="ml-2 text-2xl font-normal text-base-content/70">
                                                            ({movie.release_date.split('-')[0]})
                                                        </span>
                                                    )}
                                                </h1>
                                            ...
                                            {/* Overview with improved readability */}
                                            <div className="p-4 border shadow-inner bg-base-100/50 backdrop-blur-sm rounded-xl border-base-content/5">
                            ...
                            {showModal && (
                                <div className="fixed inset-0 z-50 flex items-center justify-center p-2 transition-all duration-300 sm:p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
                                    <div
                                    ...
                                    >
                                        <div className="absolute top-0 left-0 right-0 z-20 h-12 pointer-events-none bg-gradient-to-b from-black/70 to-transparent"></div>
                                        ...
                                        >
                                            <MdClose size={20} className="transition-transform duration-300 group-hover:rotate-90" />
                                        </button>

                                        <div className="absolute bottom-0 left-0 right-0 z-20 flex items-center h-12 px-3 pointer-events-none sm:h-16 bg-gradient-to-t from-black/70 to-transparent sm:px-4">
                                            <h3 className="text-sm font-medium text-white truncate sm:text-base text-shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                            ...
                            <div className="space-y-6 md:col-span-2">
                                {/* Movie Facts */}
                                <div className="p-6 transition-all duration-300 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-2xl border-base-content/5 hover:shadow-xl">
                                    <h2 className="flex items-center pb-3 mb-5 text-xl font-semibold border-b border-base-content/10">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 xs:grid-cols-2 md:grid-cols-3 sm:gap-6">
                                        <div className="p-3 transition-all duration-300 bg-base-100/50 sm:p-4 rounded-xl hover:bg-base-100 hover:shadow-md group">
                                            <h3 className="mb-1 text-xs font-medium transition-colors sm:text-sm text-base-content/60 group-hover:text-primary">Status</h3>
                                            <p className="text-sm font-semibold sm:text-base text-base-content">{movie.status || "N/A"}</p>
                                        </div>
                                        <div className="p-3 transition-all duration-300 bg-base-100/50 sm:p-4 rounded-xl hover:bg-base-100 hover:shadow-md group">
                                            <h3 className="mb-1 text-xs font-medium transition-colors sm:text-sm text-base-content/60 group-hover:text-primary">Release Date</h3>
                                            <p className="text-sm font-semibold sm:text-base text-base-content">{formatDate(movie.release_date)}</p>
                                        </div>
                                        <div className="p-3 transition-all duration-300 bg-base-100/50 sm:p-4 rounded-xl hover:bg-base-100 hover:shadow-md group">
                                            <h3 className="mb-1 text-xs font-medium transition-colors sm:text-sm text-base-content/60 group-hover:text-primary">Runtime</h3>
                                            <p className="text-sm font-semibold sm:text-base text-base-content">{formatRuntime(movie.runtime)}</p>
                                        </div>
                                        <div className="p-3 transition-all duration-300 bg-base-100/50 sm:p-4 rounded-xl hover:bg-base-100 hover:shadow-md group">
                                            <h3 className="mb-1 text-xs font-medium transition-colors sm:text-sm text-base-content/60 group-hover:text-primary">Budget</h3>
                                            <p className="text-sm font-semibold sm:text-base text-base-content">{formatCurrency(movie.budget)}</p>
                                        </div>
                                        <div className="p-3 transition-all duration-300 bg-base-100/50 sm:p-4 rounded-xl hover:bg-base-100 hover:shadow-md group">
                                            <h3 className="mb-1 text-xs font-medium transition-colors sm:text-sm text-base-content/60 group-hover:text-primary">Revenue</h3>
                                            <p className="text-sm font-semibold sm:text-base text-base-content">{formatCurrency(movie.revenue)}</p>
                                        </div>
                                        <div className="p-3 transition-all duration-300 bg-base-100/50 sm:p-4 rounded-xl hover:bg-base-100 hover:shadow-md group">
                                            <h3 className="mb-1 text-xs font-medium transition-colors sm:text-sm text-base-content/60 group-hover:text-primary">Original Language</h3>
                                            <p className="text-sm font-semibold sm:text-base text-base-content">{movie.original_language?.toUpperCase() || "N/A"}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {torrentLinks.length > 0 && (
                                    <div className="p-6 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-2xl border-base-content/5">
                                        <h2 className="flex items-center pb-3 mb-5 text-xl font-semibold border-b border-base-content/10">
                                        ...
                                        <div className="space-y-3">
                                            {torrentLinks.map((torrent, index) => (
                                                <div key={index}
                                                    className="flex items-center justify-between p-4 transition-all duration-300 border rounded-xl bg-base-100/70 hover:bg-base-100 border-base-content/5"
                                                >
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center px-4 py-2 space-x-2 text-sm font-medium text-white transition-all duration-300 rounded-lg bg-primary hover:bg-primary-focus"
                                                    >
                            ...
                            <div className="md:col-span-1">
                                <div className="h-full p-6 transition-all duration-300 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-2xl border-base-content/5 hover:shadow-xl">
                                    <h2 className="flex items-center pb-3 mb-5 text-xl font-semibold border-b border-base-content/10">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 mr-2 text-primary">
                                    ...
                                    <div className="space-y-2 sm:space-y-3">
                                        {cast.slice(0, 6).map((actor, index) => (
                                            <div key={index} className="flex items-center gap-2 p-2 transition-all duration-300 border group sm:gap-3 sm:p-3 rounded-xl bg-base-100/50 hover:bg-base-100 hover:shadow-md border-base-content/5">
                                                <div className="relative w-10 h-10 overflow-hidden transition-all duration-300 border-2 rounded-full shadow-sm sm:w-14 sm:h-14 border-primary/20 group-hover:border-primary/40">
                                                    <img
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-sm font-medium transition-colors duration-300 sm:text-base text-base-content group-hover:text-primary">{actor.name}</p>
                                    ...
                                    <Link
                                        href={`/movies/${id}/cast`}
                                        className="flex items-center justify-center w-full gap-2 px-4 py-3 mt-5 font-medium transition-all duration-300 border bg-base-100 hover:bg-base-100/80 text-primary rounded-xl hover:shadow-md group border-primary/10 hover:border-primary/30"
                                    >
                        ...
                        {similarMovies.length > 0 && (
                            <div className="mt-12">
                                ...
                                <div className="p-3 transition-all duration-300 border shadow-lg bg-base-200/80 backdrop-blur-sm rounded-2xl sm:p-4 md:p-6 border-base-content/5 hover:shadow-xl">
                                    {/* Desktop/Tablet Grid View */}
                                    <div className="hidden grid-cols-2 gap-3 sm:grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 sm:gap-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-3 sm:hidden xs:grid-cols-2">
                                ...
                                <div className="mt-4 text-center md:hidden">
                                    <Link
                                        href="/movies"
                                        className="inline-flex items-center justify-center gap-2 px-6 py-3 transition shadow-md bg-primary text-primary-content rounded-xl hover:bg-primary/20"
                                    >
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[70vh]">
                        <div className="mb-4 text-4xl font-bold text-base-content/30">Movie Not Found</div>
                        <p className="mb-8 text-base-content/70">The movie you're looking for doesn't exist or has been removed.</p>
                        <Link href="/movies" className="px-6 py-3 transition rounded-full shadow-md bg-primary text-primary-content hover:bg-primary/90">
                            Back to Movies
                        </Link>
                    </div>
                )}
                ...
                {playingMovie && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/90 backdrop-blur-sm">
                        <div className="relative z-10 w-full h-full overflow-hidden rounded-lg shadow-2xl max-w-screen-full sm:rounded-xl">
                            <button
                                onClick={() => setPlayingMovie(false)}
                                className="absolute z-20 p-2 text-white transition-colors rounded-full top-2 sm:top-4 right-2 sm:right-4 bg-black/50 hover:bg-black/70"
                            >
                                <MdClose size={24} />
                            </button>
