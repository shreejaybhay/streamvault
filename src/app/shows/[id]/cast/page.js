...
const ShowCastPage = ({ params: { id: showId } }) => {
    ...
    useEffect(() => {
        const fetchShowData = async () => {
            ...
            try {
                ...
                const [castResponse, showResponse] = await Promise.all([
                    fetch(`https://api.themoviedb.org/3/tv/${showId}/credits?api_key=${apiKey}`),
                    fetch(`https://api.themoviedb.org/3/tv/${showId}?api_key=${apiKey}`)
                ]);

                const [castData, showData] = await Promise.all([
                    castResponse.json(),
                    showResponse.json()
                ]);
                    ...
                    <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
                        ...
                        <Link href={`/shows/${showId}`}>
                            ...
                            >
                                ...
                                <span className="text-primary font-medium">{showTitle}</span>
                            </motion.div>
                        </Link>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {filteredAndSortedCast.map((actor, index) => (
                            <Link 
                                href={`/shows/${showId}/cast/${actor.id}`}
                                key={actor.id}
                            >
                                <motion.div
                                    initial={{ opacity: 0, y: 20 }}
